...
def generate_and_save_gradcam(model, input_tensor, target_layers, original_image, output_dir, target_classes=None, label='', predicted_label=''):
    """
    Generate and save Grad-CAM visualizations for multiple classes.
    
    Args:
        model: Trained PyTorch model.
        input_tensor: Preprocessed input tensor of shape [1, C, H, W].
        target_layers: List of model layers for GradCAM analysis.
        original_image: Original image (numpy array, unnormalized).
        output_dir: Directory to save GradCAM results.
        target_classes: List of target class indices. If None, uses all classes.
    """
...
def visualize_with_gradcam(model, dataset, transform, output_dir, target_layers, device):
    """
    Visualize Grad-CAM for a set of images in the dataset, including predicted labels in the folder name.
    Args:
        model: Trained PyTorch model.
        dataset: Dataset to visualize (e.g., test or validation set).
        transform: Preprocessing transformations applied to images.
        output_dir: Directory to save GradCAM visualizations.
        target_layers: List of model layers for GradCAM.
...
class RetinopathyDataset(Dataset):
    ...
    def ben_graham_preprocessing(self, img):
        """Ben Graham preprocessing - resize image to a circular crop."""
        size = min(img.size)
        img = ImageOps.fit(img, (size, size), Image.Resampling.LANCZOS)  # Use LANCZOS instead of ANTIALIAS
...
transform_train_original = transforms.Compose([
    ...
    transforms.ColorJitter(brightness=(0.1, 1.9)),
    transforms.ToTensor(),   
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
...
def get_meta_model(base_model_preds, true_labels):
    """
    Trains and returns a meta model using the predictions from base models.

    Args:
        base_model_preds (np.ndarray): Predictions from base models as a matrix of shape (num_samples, num_models).
        true_labels (np.ndarray): Ground truth labels.

    Returns:
        meta_model: Trained meta model (Logistic Regression).
    """
...
def optimize_weights(base_model_preds, true_labels):
    """
    Optimizes and returns the weights for the weighted average ensemble method.
    ...
    Returns:
        weights (list): Optimized weights for each base model.
    """
...
def weighted_average(predictions, true_labels):
    """
    Computes a weighted average ensemble using optimized weights.

    Args:
        predictions (list of np.array): List of prediction arrays from base models.
...
def boosting_ensemble(predictions, true_labels):
    """
    Boosting ensemble method using Gradient Boosting Classifier.
...
def compare_models(val_pred_paths):
    """
    Compare all ensemble methods using Cohen's kappa score.

    Args:
        val_pred_paths (list of str): List of file paths to prediction CSVs.

    Returns:
        dict: Cohen's kappa scores for each method.
    """
    # Load true labels
                ...
                for train_transform in train_transforms:
                    for attention in attentions:
                        ...
                        for loop in range(loops):
                            ...
                            target_layers = [model.backbone.features[-1]]  # Adjust to match your model
...
#   }
    ...
    # attention = ['self', 'channel', 'spatial']
    ...
    #val_pred_paths = ['./outputs/task3/taskb_quick/model_dual_resnet18_freezeFalse_transformoriginal_none_0.csv', './outputs/task3/taskb_quick/model_single_vgg16_freezeFalse_transformoriginal_none_0.csv', './outputs/task3/taskb_quick/model_single_densenet121_freezeFalse_transformoriginal_none_0.csv']
    #compare_ensemble(val_pred_paths=val_pred_paths)


    # remember correct version
    val_pred_paths = [
    ...
    scores = compare_models(val_pred_paths)
    ...
    compare_training(
        ...
        state_dict_path = './pretrained_DR_resize/pretrained/densenet121.pth',
        attentions = ['self'],
        subfolder_name='SelfAttentionDN',
        backbone_names=["densenet121"],
        modes=['single'],
        backbone_freezes=[False],
        train_transforms={"transformoriginal": transform_train_original},
        loops=6,
    ...
    # compare_training(
    #     dataset_name = 'DeepDRiD/',
    #     state_dict_path = './pretrained_DR_resize/pretrained/resnet18.pth',
    #     attentions = ['none'],
    #     subfolder_name='taskb',
    ...
    #     dataset_name = 'DeepDRiD/',
    #     state_dict_path = './outputs/task2/single_vgg_unfrozen_transoriginal_kaggleDRresized/model_single_vgg16_freezeFalse_transformoriginal_0.pth',
    #     attentions = ['spatial', 'none', 'self', 'channel'],
    #     subfolder_name='taskb-freeze',
    #     backbone_names=["vgg16"],
    #     modes=['single'],
    #     backbone_freezes=[True],
    #     train_transforms={"transformoriginal": transform_train_original},
    #     loops=6,
    #     #preprocessing_options=preprocessing_options
    #     )
    ...
    #     state_dict_path = './outputs/task2/single_vgg_unfrozen_transoriginal_kaggleDRresized/model_single_vgg16_freezeFalse_transformoriginal_0.pth',
    #     attentions = ['spatial', 'none', 'self', 'channel'],
    #     subfolder_name='taskb-freeze_pre',
    #     backbone_names=["vgg16"],
    #     modes=['single'],
    #     backbone_freezes=[True],
    #     train_transforms={"transformoriginal": transform_train_original},
